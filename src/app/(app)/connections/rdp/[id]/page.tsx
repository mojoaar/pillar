...
import RdpViewer from '@/components/terminal/RdpViewer';
import Link from 'next/link';
import { ArrowLeft, Monitor, ShieldCheck } from 'lucide-react';

interface RdpPageProps {
  params: Promise<{ id: string }>;
}

export default async function ConnectionRdpPage({ params }: RdpPageProps) {
  ...
  return (
    <div style={{
        ...
        <div className="flex-align-center">
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <div className="flex-align-center">
              ...
              <strong style={{ fontSize: '0.95rem' }}>{connection.name} (RDP)</strong>
      ...
      {/* RDP Viewer canvas Display Area */}
      <div style={{ flex: 1, minHeight: 0, position: 'relative' }}>
        <RdpViewer connectionId={id} />
      </div>
    </div>
  );
}
